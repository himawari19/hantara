...
export default function RegisterPage() {
  ...
  const { signUpWithEmail, signInWithGithub, signInWithGoogle, isLocalMode } = useAuthStore();
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (authError) {
      ...
      // Supabase mode — show email confirmation
      setSuccess(true);
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--bg-primary)] px-4">
        <div className="w-full max-w-sm text-center">
          <div className="rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] p-8">
            <div className="mb-4 text-4xl">✉️</div>
            <h2 className="mb-2 text-lg font-bold text-[var(--text-primary)]">
              Check your email
            </h2>
            <p className="mb-6 text-sm text-[var(--text-secondary)]">
              We sent a confirmation link to <strong className="text-[var(--text-primary)]">{email}</strong>. Click the link to activate your account.
            </p>
            <Link
              href="/login"
              className="inline-block rounded-lg bg-[var(--accent)] px-6 py-2.5 text-sm font-bold text-white hover:bg-[var(--accent-hover)]"
            >
              Go to Login
            </Link>
          </div>
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] p-6">
          ...
          <form onSubmit={handleRegister} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="reg-email" className="text-xs text-[var(--text-secondary)]">
                Email
              </label>
              <input
                id="reg-email"
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="reg-password" className="text-xs text-[var(--text-secondary)]">
                Password
              </label>
              <input
                id="reg-password"
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="confirm-password" className="text-xs text-[var(--text-secondary)]">
                Confirm Password
              </label>
              <input
                id="confirm-password"
